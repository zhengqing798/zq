...
>
> **页面上不放"描述性文字"**：口径说明、数据来源、数据规模、已知局限等说明文字**一律不出现在界面**上
> （接口仍返回、`docs/` 仍记录、测试仍守护），页面只保留功能与数据本身。渲染脚本的反向断言会把
> `口径说明`/`数据来源`/`每个结论都带`/`按岗位数排序`/`如实说明`/`PBKDF2` 等字样当作"不该出现的内容"扫描全量 DOM。
