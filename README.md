...
│   ├── 数据预处理文档.md         # 任务3：清洗与特征工程（含薪资列规范化）
...
│   └── AI使用说明与反思报告.md    # 任务14
├── data/                      # ★ 数据（原始+清洗+分词结果均已入库）
│   ├── raw/                   # 任务2：爬取原始数据（≥1万条）
│   ├── processed/             # 任务3：清洗、分词、薪资规范化、简历结构化结果
│   └── external/              # 辅助数据（停用词表、行业词典、城市坐标等）
├── src/                       # ★ 源码（按任务分包）
│   ├── crawler/               # 任务2：智联招聘爬虫（Selenium）
│   ├── preprocessing/         # 任务3：清洗 + 简历结构化 + 薪资规范化
│   ├── analysis/              # 任务4：可视化与数据看板（输出到 reports/）
│   ├── models/
│   │   ├── scoring/           # 任务5：评分模型（待重新训练）
│   │   ├── matching/          # 任务6：人岗匹配
│   │   └── clustering/        # 任务7：聚类
│   ├── rag/                   # 任务10：向量库 + RAG 检索
