...
def skill_score(hits, n_req, n_res):
    ...
    return round((0.7 * cov + 0.3 * util) * 100, 2), round(cov * 100, 2), round(util * 100, 2)
...
def write_report(out, diag, jobs, resumes, audit, black, indus, n_syn_strict, n_loose, n_same):
    ...
    L.append("| 维度 | 权重 | 算分规则 |\n|---|---|---|")
    L.append("| 技能 | 0.30 | `0.7 × 岗位技能覆盖率 + 0.3 × 简历技能利用率`。岗位技能要求集合 = 岗位 `技能标签`（**剔除福利类 %d 个 + 行业类 %d 个标签**）∪ 职位描述中出现的简历技能池词；匹配前做保守档同义词归一；**集合为空 → 0 分**（无技能证据即视为不匹配，不给中性分） |"
             % (len(black), len(indus)))
